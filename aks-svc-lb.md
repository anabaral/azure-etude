# AKS 에서 Load Balancer 사용

## 일반적인 Kubernetes에서의 Load Balancer 사용 방식들

AKS 등 Kubernetes 에서 Load Balancer 를 사용하는 방법은 다음과 같이 세 가지로 나눌 수 있다:
...
- 그림 왼쪽, 서비스 타입을 LoadBalancer로 설정하면 어느 포트를 잡아서 그 포트로 접속하면 서비스를 접근할 수 있다.  
  클러스터 통틀어 하나의 서비스만 사용한다면 80/443 포트로 서비스하면 오케이.
...
- 그림 오른쪽, Load Balancer 에 해당하는 기능을 벤더 제공 솔루션이 대체한다.  
  이 방식을 하면 말 그대로 부하 분산기 CPU/Memory 자원이 클러스터 외부에 존재하기에 부담이 나누어지며, 당연히 과금도 별도로 일어난다.
